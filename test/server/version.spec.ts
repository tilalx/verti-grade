...
describe('parseAppVersion', () => {
  ...
  // Regression: this is what `git describe --tags --always | sed 's/^v//'`
  // emits on every non-release build, and the previous anchored regex
  // rejected it — silently disabling the whole update check.
...
describe('compareSemver', () => {
  ...
  // The old comparator did Number(...) || 0, so any non-numeric segment
  // silently became 0 and a suffixed tag compared equal to its base.
...
describe('decideUpdate', () => {
  ...
  it('uses the authoritative ahead count, not the git-describe hint', () => {
    // git describe said "-7-", but the compare API reports it is level.
