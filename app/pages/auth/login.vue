          ...
          <template v-if="authMethods.oauth2?.enabled">
            ...
            <v-row dense>
              ...
              >
                ...
                >
                  <v-icon start size="16">{{ providerIcon(p.name) }}</v-icon>
                  {{ p.displayName }}
                </v-btn>
              </v-col>
            </v-row>
...
const hasAnyAuth  = !!(authMethods?.password?.enabled || authMethods?.oauth2?.enabled)
...
const identityLabel        = computed(() => supportsEmail && supportsUser ? t('account.username_or_email') : supportsEmail ? t('account.email') : t('account.username'))
const identityInputType    = computed(() => supportsEmail && !supportsUser ? 'email' : 'text')
const identityAutocomplete = computed(() => supportsEmail ? 'email' : 'username')
const identityIcon         = computed(() => supportsEmail && !supportsUser ? 'mdi-email-outline' : 'mdi-account-outline')

// ── View meta ──────────────────────────────────────────────────────
const viewEyebrow  = computed(() => ({ login: 'WELCOME BACK', requestReset: 'ACCOUNT RECOVERY' }[view.value] ?? ''))
const viewTitle    = computed(() => ({ login: t('account.login'), requestReset: t('account.reset_password') }[view.value] ?? ''))
...
const PROVIDER_ICONS = {
  apple: 'mdi-apple', google: 'mdi-google', microsoft: 'mdi-microsoft',
  facebook: 'mdi-facebook', github: 'mdi-github', gitlab: 'mdi-gitlab',
  discord: 'mdi-discord', twitter: 'mdi-twitter', spotify: 'mdi-spotify',
  twitch: 'mdi-twitch', bitbucket: 'mdi-bitbucket',
  oidc: 'mdi-lock', oidc2: 'mdi-lock', oidc3: 'mdi-lock',
}
const providerIcon = name => PROVIDER_ICONS[name] ?? 'mdi-login'

// ── Helpers ────────────────────────────────────────────────────────
async function validate(ref) {
  const f = ref?.value; if (!f) return true
  const { valid } = await f.validate(); return valid
}

function detectCapsLock(ev) {
  if (typeof ev.getModifierState === 'function') capsLockOn.value = ev.getModifierState('CapsLock')
}

watch(view, async () => { await nextTick(); document.querySelector('input')?.focus() })

function resolveAuthError(err) {
  const msg = err?.data?.message ?? err?.message ?? ''
  if (/invalid.+credentials/i.test(msg)) return t('notifications.error.invalid_credentials')
  if (/not verified/i.test(msg))         return t('notifications.error.email_not_verified')
...
async function submitLogin() {
  ...
  loading.value = true
  try {
    await pb.collection('users').authWithPassword(identity.value, password.value, { autoRefreshThreshold: 0 })
...
async function submitReset() {
  ...
  try {
    ...
    view.value = 'login'; resetEmail.value = ''
  } catch (err) { layout.value.notify(resolveAuthError(err), 'error') }
  finally { loading.value = false }
}
